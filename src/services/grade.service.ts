...
export class GradeService {
  ...
  async calculateStudentGPA(studentId: string): Promise<{ gpa: number; totalCredits: number }> {
    const result = await this.getStudentGrades(studentId);
    const records = result.items;

    if (records.length === 0) return { gpa: 0, totalCredits: 0 };

    const totalPoints = records.reduce((sum: number, r: Record<string, unknown>) => {
      const gp = r.gradePoint;
      return sum + parseFloat(typeof gp === 'string' ? gp : '0');
    }, 0);

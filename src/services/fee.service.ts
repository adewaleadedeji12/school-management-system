...
export class FeeService {
  ...
  async payFee(feeId: string, amount: number, paymentMode: string, transactionId?: string) {
    ...
    const currentPaid = parseFloat(typeof fee.amountPaid === 'string' ? fee.amountPaid : '0');
    const totalAmount = parseFloat(typeof fee.totalAmount === 'string' ? fee.totalAmount : '0');
  ...
  async getFeeSummary() {
    ...
    const fees = all.items as Record<string, unknown>[];

    const parseNum = (val: unknown): number => parseFloat(typeof val === 'string' ? val : '0');

    return {
      totalCollected: fees
        .filter((f) => f.status === 'paid')
        .reduce((sum, f) => sum + parseNum(f.amountPaid), 0),
      totalPending: fees
        .filter((f) => ['pending', 'partial'].includes(f.status as string))
        .reduce((sum, f) => sum + parseNum(f.balance), 0),
      totalOverdue: fees
        .filter((f) => f.status === 'overdue')
        .reduce((sum, f) => sum + parseNum(f.balance), 0),
      paidCount: fees.filter((f) => f.status === 'paid').length,
      pendingCount: fees.filter((f) => ['pending', 'partial'].includes(f.status as string)).length,
    };
  }
}

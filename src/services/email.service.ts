...
export class EmailService {
  ...
  private async sendBrevoEmail(payload: BrevoEmailPayload): Promise<boolean> {
    if (!this.apiKey) {
      logger.warn('Cannot send email: BREVO_API_KEY not configured');
      return false;
    }
    ...
    try {
      ...
      logger.info(
        { messageId: (data as Record<string, unknown>).messageId, to: payload.to[0]?.email },
        'Email sent successfully via Brevo REST API'
      );
      return true;
    } catch (error) {
      logger.error({ error, to: payload.to[0]?.email }, 'Failed to send email');
      return false;
    }
  }
  ...
  async sendEmail(options: EmailOptions): Promise<boolean> {
    ...
    return this.sendBrevoEmail(payload);
  }
